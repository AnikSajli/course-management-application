...
namespace Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<Teacher> GetTeacherByPhone(string phone)
        {
            var teacher = await _context.Teachers.FirstOrDefaultAsync(u => u.PhoneNumber == phone);
            return teacher;
        }
        ...
        {
            var student = await _context.Students.FirstOrDefaultAsync(u => u.PhoneNumber == phone);
            return student;
        }

        public async Task<Student> GetStudentByRoll(int roll)
        {
            var student = await _context.Students.FirstOrDefaultAsync(u => u.Roll == roll);
            return student;
        }

        public async Task<Course> GetCourseByCode(string courseCode)
        {
            var course = await _context.Courses.FirstOrDefaultAsync(u => u.CourseCode == courseCode);
            return course;
        }
    }
}
